...
HEADER_FIELDS = {
    ...
    "gross_weight":        "Gross Weight (MT)",
    "tare_weight":         "Tare Weight (MT)",
    "net_weight":          "Net Weight (MT)",
...
def invoice_to_snapshot(inv: Any) -> dict:
    ...
    return {
        ...
        # Weights stored in kg; emit as MT (3 decimals) so the diff UI shows MT.
        "gross_weight": round(float(inv.gross_weight) / 1000, 3) if inv.gross_weight else None,
        "tare_weight": round(float(inv.tare_weight) / 1000, 3) if inv.tare_weight else None,
        "net_weight": round(float(inv.net_weight) / 1000, 3) if inv.net_weight else None,
