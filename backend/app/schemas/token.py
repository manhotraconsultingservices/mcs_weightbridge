...
class TokenCreate(BaseModel):
    ...
    royalty_unit: Optional[str] = None   # 'mt' | 'cum' — royalty basis (operator-selected)
    royalty_rate: Optional[Decimal] = None   # operator ₹/unit override (else product master rate for the unit)
...
class ProductBrief(BaseModel):
    ...
    royalty_per_mt: Decimal | None = None  # ₹/MT master rate — prefills the token royalty field
    royalty_per_cum: Decimal | None = None # ₹/CUM master rate — prefills the token royalty field
    model_config = {"from_attributes": True}
...
class TokenResponse(BaseModel):
    ...
    royalty_unit: Optional[str] = None   # 'mt' | 'cum' — royalty basis (operator-selected)
    royalty_rate: Optional[Decimal] = None   # ₹/unit rate used (operator override or product master)
    royalty_amount: Optional[Decimal] = None # computed royalty charge (rate × qty)
