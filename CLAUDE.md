...
**Auto-sync on finalise:** when `tally_config.auto_sync` + `is_enabled`, finalising a GST `sale`/`purchase` invoice fires a fire-and-forget `BackgroundTask` (`invoices.py::_auto_sync_tally_bg`) that opens its own tenant-routed session, reuses `tally.py::_push_invoice`, and commits `tally_synced`. Non-blocking, never rolls back finalisation, leaves `tally_synced=False` on failure so the invoice stays in `/tally/pending` for manual retry. Toggle in Settings → Tally.
